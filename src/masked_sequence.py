...
# ---------------- Config ----------------
DATA_FOLDERS  = ['Ninapro/DB2_emg_only/split set']
TEST_FOLDERS  = ['Ninapro/vis_sample']
WINDOW_LEN    = 400
STEP          = 100
MASK_RATIO    = 0.2
BATCH_SIZE    = 2048
...
class MaskedEMGMAE(nn.Module):
    ...
    def block_mask(self, x, num_seg=NUM_SEG, seg_len=SEG_LEN):
        """
        随机分段掩码
        """
        B, C, T = x.shape
        ...
        for b in range(B):
            for _ in range(num_seg):
                # 随机选择 num_seg 个起始点
                start = torch.randint(0, max(1, T-seg_len), (1,), device=x.device)
                mask[b, :, start:start + seg_len] = True
        xm = x.masked_fill(mask.expand_as(x), 0.)
        return xm, mask
    

    def freq_mask(self, x, freq_band='high'):
        Xf = torch.fft.rfft(x, dim=-1)
        B, C, F = Xf.shape
        half = F // 2
        if freq_band == 'high':
            mask_idx = torch.arange(half, F, device=x.device)
        else:
            mask_idx = torch.arange(0, half, device=x.device)
        
        mask = torch.zeros_like(Xf, dtype=torch.bool)
        mask[..., mask_idx] = True

        Xf = Xf.masked_fill(mask, 0.)
        xm = torch.fft.irfft(Xf, n=x.shape[-1], dim=-1)
        return xm, mask[..., :xm.shape[-1]]

    def forward(self, x):
        if MASK_TYPE == 'block':
            xm, mask = self.block_mask(x)
        elif MASK_TYPE == 'freq':
            xm, mask = self.freq_mask(x, freq_band=FREQ_BAND)
        else:
...
def test():
    ...
    # 加载 encoder 权重
    ckpt = torch.load('Ninapro/full_model.pth', map_location=DEVICE)
